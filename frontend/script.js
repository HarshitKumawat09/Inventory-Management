...
// Function to manually add data to the purchase orders table
function addManualPurchaseOrders() {
    const manualOrders = [
        {
            po_number: "PO12345",
    ...
    // Iterate over each manually defined order and add a row to the table
    ...
    alert("Manual Purchase Orders added successfully!");
}

// Event listener to call addManualPurchaseOrders when needed
document.addEventListener("DOMContentLoaded", () => {
    // Populate tables from the backend
    populateProductsTable();
    populateVendorsTable();
    populatePurchaseOrdersTable();
    populateShipmentsTable();

    // Manually add purchase orders after the page loads
    addManualPurchaseOrders();  // This will add the manual data when the page loads
});
